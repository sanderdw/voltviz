...
### Added
- Open Graph / Twitter card image (`og-image.png`, 1200×630) plus `og:locale`, `og:image:*` and `twitter:image` tags, so links to voltviz.com render a proper preview. Generated by `npm run make:og-image`.
- `robots.txt`, `sitemap.xml`, `humans.txt`, `llms.txt` and `/.well-known/security.txt` (RFC 9116) served from the site root.
- Maskable 512×512 PWA icon.

### Fixed
- VoltViz no longer shares its Sendspin identity with the Music Assistant web player. `@sendspin/sendspin-js` persists the client's identity keypair in `localStorage` under fixed, un-namespaced keys (`sendspin-identity-sk`, `sendspin-psks`, ...), and `player.clientId` (the base64url public key) is the Music Assistant player id. Music Assistant's own frontend uses the same SDK with the same defaults, so when both run as Home Assistant add-ons under Ingress they are same-origin, share that key, and register as *one* MA player whose name flipped between "VoltViz" and "Web (Chrome on Windows)" every time either tab (re)connected — MA unregisters and re-creates the player on each connect of an existing client id — freezing playback whenever both tabs were open. VoltViz now passes its own `storage` adapter that prefixes every SDK key with `voltviz:`, giving it a separate, still persistent identity and therefore its own MA player. The old un-prefixed key is deliberately not migrated (under Ingress it belongs to the MA web player), so standalone (non-Ingress) deployments register as a new MA player once; the stale previous "VoltViz" player can be removed in Music Assistant's player settings.
- Icons moved from `fav/` to `public/`, so `/favicon.ico`, `/apple-touch-icon.png` and `/site.webmanifest` exist at the site root instead of being hashed into `assets/`. The manifest previously shipped empty `name`/`short_name`, a white theme color and icon paths that 404'd in production; it now uses relative paths that also work under Home Assistant ingress and is served as `application/manifest+json`.
- nginx sends `X-Content-Type-Options: nosniff` and `Referrer-Policy` headers and serves `/.well-known/` files explicitly (no SPA fallback).
