...
- Keep `CHANGELOG.md` up to date for every user-visible change (features, fixes, behavior changes, docs/process changes that affect contributors).
- When making a change, add a short bullet under `## [Unreleased]` in `CHANGELOG.md` using the appropriate section (`Added`, `Changed`, `Fixed`, `Removed`, `Security`). Create the section if it does not exist.
- If no changelog entry is needed (pure refactor/chore with no user or contributor impact), explicitly state that in the final response.
